...
<div class="card">	
	<div class="card-header">  
		<h4 class="card-title">Entry Data Cacat</h4>
	</div>
	<div class="card-block">
	   <div class="container">
            <form action="{{ route('cacat.store') }}" method="post">
                 {{ csrf_field() }}

                 <div class="form-group row {{ $errors->has('nm_cacat') ? 'has-danger' : '' }}">
                     <label class="col-sm-2 col-form-label">Nama Cacat</label>
                     <div class="col-sm-4">
                        <input type="text" name="nm_cacat" class="form-control {{ $errors->has('nm_cacat') ? 'form-control-danger' : '' }}" value="{{ old('nm_cacat') }}">
