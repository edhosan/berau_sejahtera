...
<div class="card">	
	<div class="card-header">  
		<h4 class="card-title">Ubah Data Akseptor KB</h4>
	</div>
	<div class="card-block">
	   <div class="container">
            <form action="{{ route('akseptor.update', $akseptor) }}" method="post">
                 {{ csrf_field() }}

                 <div class="form-group row {{ $errors->has('nm_akseptor') ? 'has-danger' : '' }}">
                     <label class="col-sm-2 col-form-label">Nama Akseptor KB</label>
                     <div class="col-sm-4">
                        <input type="text" name="nm_akseptor" class="form-control {{ $errors->has('nm_akseptor') ? 'form-control-danger' : '' }}" value="{{ $akseptor->nm_kb or old('nm_akseptor') }}">
